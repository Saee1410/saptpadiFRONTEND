...
const UserProfile = () => {
    ...
    const fetchData = async (id) => {
        try {
            const [userRes, bookingRes] = await Promise.all([
                ...
                axios.get(`http://localhost:5000/api/bookings/my-bookings/${id}`)
            ]);
    ...
    const handleCancelBooking = async (bookingId) => {
        ...
        try {
            const res = await axios.put(`http://localhost:5000/api/bookings/cancel/${bookingId}`);
            if (res.data.success) {
    ...
    const handleUpdateProfile = async () => {
        try {
            ...
            const res = await axios.put(`http://localhost:5000/api/users/update-user/${userId}`, formData);
            if (res.data.success) {
