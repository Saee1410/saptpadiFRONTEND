...
const VendorProfile = () => {
    ...
    useEffect(() => {
        const fetchVendorProfile = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5000/api/bookings/vendor/${vendorId}`);
    ...
    try {
        ...
        const res = await axios.put(`http://localhost:5000/api/bookings/update-status/${bookingId}`, {
            status: 'Accepted' // नेहमी 'Accepted' कॅपिटल 'A' ने पाठवा
        });
    ...
    const handleUpdateProfile = async () => {
        try {
            const vendorId = user._id || user.id;
            const res = await axios.put(`http://localhost:5000/api/users/update-vendor/${vendorId}`, formData);
